...
class HttpServer {
    constructor(node, blockchain, operator, minter, generator, keystore) {
        ...
        this.app.get('/miner/location', (req, res) => {
            let initialData = minter.getLocation(generator, req.body.rewardAddress, req.body['feeAddress'] || req.body.rewardAddress);

            console.log(JSON.stringify(initialData))
            // Broadcast Hash(N1), Hash(N2), Hash(Block)
            let peer = 'http://localhost:3002';
            node.broadcastInitialData(peer, {
                publicKey: keystore.keys.publicKey,
                transactionListHash: initialData.transactionListHash,
                locationListHash: initialData.locationListHash,
                hashedN1: initialData.hashedN1,
                hashedN2: initialData.hashedN2,
                blockhash: initialData.newBlockHash
            })

            res.status(201).send(initialData.location);
        })
